...
def call_openai_mini(
    ...
    return call_openai(prompt=prompt, model=OPENAI_MINI_MODEL, temperature=temperature,
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    table = []
    table.append(f"{'Prompt':<40} {'GPT-4o Response':<40} {'Mini Response':<40} {'GPT-4o Latency'} {'Mini Latency'}")
    table.append("-" * 160)
    
    for res in results:
        prompt = res.get("prompt", "")[:40] if len(res.get("prompt", "")) > 40 else res.get("prompt", "")
        gpt4o_resp = res.get("gpt4o_response", "")[:40] if len(res.get("gpt4o_response", "")) > 40 else res.get("gpt4o_response", "")
        mini_resp = res.get("mini_response", "")[:40] if len(res.get("mini_response", "")) > 40 else res.get("mini_response", "")
        
        gpt4o_lat = f"{res.get('gpt4o_latency', 0):.3f}s"
        mini_lat = f"{res.get('mini_latency', 0):.3f}s"
        
        table.append(f"{prompt:<40} {gpt4o_resp:<40} {mini_resp:<40} {gpt4o_lat} {mini_lat}")
